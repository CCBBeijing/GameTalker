...
get_interaction <- function(data, col, reduction = F ){
  ...
  fit_res <- cv.glmnet(x = x_matrix, y = m,alpha = 0.2,
...
result1_H = qsODE_all(dat, 1, c(4,5,6,8), Time = TT1,LOP_order = 3)#
result2_H = qsODE_all(dat, 2, c(1,6,8), Time = TT1,LOP_order = 3)#
result3_H = qsODE_all(dat, 3, c(1,6,8), Time = TT1,LOP_order = 3)
result4_H = qsODE_all(dat, 4, c(1,6,8), Time = TT1,LOP_order = 3)
result5_H = qsODE_all(dat, 5, c(1,6,8), Time = TT1,LOP_order = 3,maxit = 1000000)
result6_H = qsODE_all(dat, 6, c(1,8), Time = TT1,LOP_order = 3)
result7_H = qsODE_all(dat, 7, c(1,6,8), Time = TT1,LOP_order = 3)
result8_H = qsODE_all(dat, 8, c(1,6), Time = TT1,LOP_order = 3)
result9_H = qsODE_all(dat, 9, c(1,6,8), Time = TT1,LOP_order = 3,maxit = 1000000)
...
nodes$name = c("sM1","sM2","sM3","sM4","sM5","sM6","sM7","sM8","sM9")
...
pdf("M13_fig2_H.pdf",height = 8,width = 8)
plot.igraph(net,
            rescale = T,
            ...
            vertex.shape="circle",
            vertex.label.cex=V(net)$ind_effect*0.7,
