...
get_interaction <- function(data, col, reduction = F ){
  ...
  fit_res <- cv.glmnet(x = x_matrix, y = m,alpha = 0.6,
...
result1_H = qsODE_all(dat, 1, c(5), Time = TT1,LOP_order = 3)#
result2_H = qsODE_all(dat, 2, c(3,4,5), Time = TT1,LOP_order = 3)#
result3_H = qsODE_all(dat, 3, c(2,4,5), Time = TT1,LOP_order = 3,maxit = 300)
result4_H = qsODE_all(dat, 4, c(3,5), Time = TT1,LOP_order = 3,maxit = 100000)
result5_H = qsODE_all(dat, 5, c(2,3,4), Time = TT1,LOP_order = 3)




result_H_log <- list()
result_H_log[[1]] <- result1_H
result_H_log[[2]] <- result2_H
result_H_log[[3]] <- result3_H
result_H_log[[4]] <- result4_H
result_H_log[[5]] <- result5_H



save(result_H_log,file="result_H_log.RData")

load("result_H_log.RData")

ode_result <- result_H_log
...
pdf("layerC_fig2_H_log.pdf",height = 8,width = 8)
plot.igraph(net,
            rescale = T,
