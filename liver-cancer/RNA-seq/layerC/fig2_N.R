...
result1_N = qsODE_all(dat, 1, c(2,5), Time = TT2,LOP_order = 3)#
result2_N = qsODE_all(dat, 2, c(5), Time = TT2,LOP_order = 3)#
result3_N = qsODE_all(dat, 3, c(2,5), Time = TT2,LOP_order = 3,maxit = 100)
result4_N = qsODE_all(dat, 4, c(2,5), Time = TT2,LOP_order = 3)
result5_N = qsODE_all(dat, 5, c(1,2,4), Time = TT2,LOP_order = 3)
...
result_N_log <- list()
result_N_log[[1]] <- result1_N
result_N_log[[2]] <- result2_N
result_N_log[[3]] <- result3_N
result_N_log[[4]] <- result4_N
result_N_log[[5]] <- result5_N





save(result_N_log,file="result_N_log.RData")

ode_result <- result_N_log
...
pdf("layerC_fig2_N_log.pdf",height = 8,width = 8)
plot.igraph(net,
            rescale = T,
